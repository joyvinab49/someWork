...
export default {
  ...
  created: function () {
    ...
    mediator.$on("oldList", (name, color) => {
      for (let i = 0; i < _this.todos.length; i++) {
        if (_this.todos[i].type[0].typeName == name) {
          _this.todos[i].type[0].typeColor = color;
          localStorage.setItem("key", JSON.stringify(_this.todos));
        }
      }
    });
    ...
    mediator.$on("typeChange", (type, index, color) => {
      _this.todos[index].type[0].typeName = type;
      _this.todos[index].type[0].typeColor = color;
      localStorage.setItem("key", JSON.stringify(_this.todos));
    });
